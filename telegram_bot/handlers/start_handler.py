...
from telegram_bot.requests_to_db.account_sync import check_user_sync, try_synchronize_accounts
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    if user_exists:
        ...
        await message.answer(f"Hi {message.from_user.first_name}, this is a bot for managing books in the library. Please enter your email and Password with which you registered in serve are your books to synchronize accounts")
...
async def stage_three(message: Message, state: FSMContext):
    ...
    await state.clear()
    if is_valid:
        await message.answer(f"accounts have synchronize successfully, please push '/start' again")
    else:
        await message.answer(f"You input wrong email or/and password, please push '/start' and try again")
...
async def all_borrowings(message: Message):
    ...
    for borrowing in borrowings:
        rented_days = borrowing["book__daily_fee"] * (timezone.now().date() - borrowing["borrow_date"]).days
        await message.answer(
