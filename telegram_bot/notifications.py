...
async def message_expired(user_id: int, book_title: str, expired_days: int) -> None:
    await bot.send_message(
        user_id,
...
async def message_no_expired(user_id: int) -> None:
    await bot.send_message(user_id, "No borrowings overdue today!")
    await bot.session.close()


def run_messages_expired() -> None:
    borrowings = Borrowing.objects.filter(
        expected_return_date__lte=timezone.now()
    ...
    users = (
        get_user_model()
        .objects.prefetch_related("borrowing")
        .exclude(telegram_id=None, borrowings__expected_return_date__lte=timezone.now())
        .distinct()
    )
    for user in users:
        user_id = user.telegram_id
        asyncio.run((message_no_expired(user_id=user_id)))
